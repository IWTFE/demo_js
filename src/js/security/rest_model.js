  ...
  var Fingerprint = new Fingerprint()
	// import {
	//   ECB_JS
	// } from './mode-ecb-min.js'
	// var {md5} = require('./_md5.js')
	// var CryptoJS = require('crypto-js/aes.js')
	// var ECB_JS = require('crypto-js/mode-ecb')
	...
	function aesEncrypt(data, deviceId) {
	  ...
	  key = md5(key).substr(8, 16);
		// console.log(key)
	  key = CryptoJS.aa.enc.Utf8.parse(key);
		console.log(key)
	  var sendData = CryptoJS.aa.enc.Utf8.parse(data);
	  var encryptedData = CryptoJS.aa.AES.encrypt(data, key, {
	    mode: CryptoJS.aa.mode.ECB,
	    padding: CryptoJS.aa.pad.Pkcs7
	  })
	...
	function aesDecrypt(data, deviceId) {
	  ...
	  key = CryptoJS.aa.enc.Utf8.parse(key);
		// console.log(key)
	  var decrypted = CryptoJS.aa.AES.decrypt(data, key, {
	    mode: CryptoJS.aa.mode.ECB,
	    padding:CryptoJS.aa.pad.Pkcs7
	  })
	  return CryptoJS.aa.enc.Utf8.stringify(decrypted).toString(); // 转换为 utf8 字符串
	}
	//获取url后参数
